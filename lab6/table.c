...
void quickSort(SET *sp, void* a[], int lo, int hi);
int partition(SET *sp, void* a[], int lo, int hi);
void* midpoint(SET *sp, void* a[], int lo, int hi);
...
/** @description: Returns a sorted copy of the elements in the table
  * @runtime: O(n)
  */
void* getElements(SET *sp) {
    ...
    quickSort(sp, elts, 0, index-1);
    return elts;
}

/** @description: Swaps two items positions in an array of generic elements
  * @runtime: O(1)
  */
void swap(void* arr[], int a, int b) {
    void* temp = arr[a];
...
/** @description: Recursive quicksort algorithm
  * @runtime: O(n)
  */
void quickSort(SET *sp, void* a[], int lo, int hi) {
    int i, last;
    // stop when size of partition < 2
    if (lo >= hi) return;
    
    // part lo to midpoint
    swap(a, lo, (lo+hi)/2);
    
	// set pivot to midpoint value placed at beginning 
    last = lo;
	
    // loop through partition
    for (i = lo+1; i <= hi; i++)
        // compare each to pivot and if less, pre-increment last and swap
        if (sp->compare(a[i], a[lo]) < 0) swap(a, ++last, i);
    // put last pivot back in
    swap(a, lo, last);
    
    // recur on left and right partitions
    quickSort(sp, a, lo, last-1);
    quickSort(sp, a, last+1, hi);
}
